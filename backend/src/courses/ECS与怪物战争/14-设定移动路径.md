# 14 设定移动路径

<div class="video-container">
  <div id="bilibili" class="video-content">
    <!-- B站嵌入：使用 https 明确协议（避免 file:// 或 http 导致被浏览器拦截） -->
    <iframe
      class="video-frame"
      src="https://player.bilibili.com/player.html?bvid=BV1hUk2BsECV&page=1&autoplay=0&danmaku=0&high_quality=1"
      width="100%"
      height="480"
      scrolling="no"
      frameborder="0"
      allowfullscreen>
    </iframe>
  </div>
</div>

[在 Bilibili 上观看](https://www.bilibili.com/video/BV1hUk2BsECV)

## 📖 概述

前两节我们把关卡从 Tiled 载入到 ECS 世界里，并补齐了背景色、渲染顺序与 gid 翻转等细节。到这里“地图能显示”了，但塔防玩法的第一件事其实是：

> 敌人要从起点出发，沿着预设路线走到终点。

这一节我们先不急着“刷怪/战斗”，而是把**移动路径（Path / Waypoints）**的数据结构与最小运行闭环搭出来：

- 在 Tiled 中用“点对象”画出路径节点，并用属性把节点连成图（支持分叉）
- 关卡加载时解析这些点对象，生成 `waypoint_nodes_`（节点表）与 `start_points_`（起点列表）
- 为敌人引入 `EnemyComponent`（目标节点 id + 移动速度）
- 新增 `FollowPathSystem`：每帧根据路径节点更新敌人的速度，让 `MovementSystem` 驱动位移

<img src="https://theorhythm.top/gamedev/MW/怪物战争.061.webp" style='width: 800px;' />

> PPT 第 61 页：第 14 节标题页

本节对应代码标签：`14-设定移动路径`（基线：`13-完善关卡载入器`）。

## 🎯 学习目标

- 理解两种路径表示：单一路径（`vector<point>`）与分叉路径（节点图）
- 定义路径节点结构 `WaypointNode { id, position, next_node_ids }`，并维护 `unordered_map<int, WaypointNode>`
- 通过自定义 Builder（`EntityBuilderMW`）在关卡加载阶段解析路径点对象
- 实现 `FollowPathSystem`：到达节点后切换下一个节点（若有多个则随机选择），到达终点则发送事件并标记删除
- 引入“延迟删除”模式：`DeadTag` + `RemoveDeadSystem`

## 🧠 1) 路径数据结构：为什么不用一个 vector 走到底？

如果只有一条固定路线，最简单的结构确实是：

- `std::vector<glm::vec2> waypoints;`
- 敌人保存当前下标 `i`，到点了就 `i++`

但塔防里经常出现 **分叉路径**（例如多条路汇聚到基地），这时仅靠“下标递增”就不够了。更通用的方式是把路径看成一张有向图：

- 每个节点有一个 `id`
- 节点知道自己可以走向哪些“下一节点”（列表）
- 可能有多个起点（多个出怪口）

<img src="https://theorhythm.top/gamedev/MW/怪物战争.062.webp" style='width: 800px;' />

> PPT 第 62 页：路径结构建议：单一路径 vs 带分叉路径；敌人组件保存目标节点 id 与移动速度

项目里选择了“分叉友好”的表示：

```cpp
// src/game/data/waypoint_node.h
struct WaypointNode {
    int id_;
    glm::vec2 position_;
    std::vector<int> next_node_ids_;
};
```

并在 `GameScene` 中维护两份运行时数据：

```cpp
// src/game/scene/game_scene.h（节选）
std::unordered_map<int, game::data::WaypointNode> waypoint_nodes_;
std::vector<int> start_points_;
```

## 🗺️ 2) 在 Tiled 里画路径：用“点对象 + 属性”连成图

本节把路径放在地图的一个对象图层里（layer 名叫 `path`），每个节点都是一个 **point 对象**（`.tmj` 中会出现 `"point":true`）。

关键是“如何连线”：我们用对象属性把一个节点指向下一个节点。

### 2.1 单分支：`next` 属性（type: object）

`assets/maps/level1.tmj` 中，一个最简单的节点长这样：

```json
{
  "id":67,
  "point":true,
  "properties":[
    { "name":"next", "type":"object", "value":68 },
    { "name":"start", "type":"bool", "value":true }
  ],
  "x":-54.6666666666667,
  "y":993.333333333333
}
```

解释：

- `id`：Tiled 为对象分配的唯一编号（我们直接拿它当 waypoint id）
- `next`：对象类型属性，`value` 会填“目标对象的 id”
- `start`：可选，标记这个节点是一个起点（出怪口）

### 2.2 多分支：`next / next2 / next3 ...`

为了支持分叉，本节约定：**属性名只要以 `next` 开头就算一条出边**。

例如某个节点同时有 `next` 与 `next2`：

```json
{
  "id":79,
  "point":true,
  "properties":[
    { "name":"next",  "type":"object", "value":86 },
    { "name":"next2", "type":"object", "value":80 }
  ]
}
```

这样一个节点就能指向多个下一节点，系统里可以随机选一条路（后面会看到实现）。

> 小提醒：对象 `id` 是 Tiled 自动维护的。一般“改位置/改属性”不会变，但如果你删除对象再新建，id 可能会变化；这会影响 `next` 的引用关系。

## 🧱 3) 关卡加载阶段解析路径：EntityBuilderMW（再次使用 Builder 可替换能力）

第 12 节我们引入了 Builder：`LevelLoader` 负责“解析 JSON”，Builder 负责“把数据变成实体/组件”。

这节我们更进一步：**游戏侧需要解析“自定义的形状对象”（路径点）**，但引擎的 `BasicEntityBuilder` 并不知道“point + next/start”是什么意思。

因此我们为怪物战争写了一个自定义 Builder：`EntityBuilderMW`，它继承 `BasicEntityBuilder` 并额外持有两份引用：

```cpp
// src/game/loader/entity_builder_mw.h（节选）
std::unordered_map<int, game::data::WaypointNode>& waypoint_nodes_;
std::vector<int>& start_points_;
```

### 3.1 override build：形状对象走 buildPath，其它对象走父类

`LevelLoader` 在对象层解析时，遇到 `gid == 0` 会走“形状对象”分支：`configure(&object)->build()`。

我们的策略是：

- `object_json_ && !tile_info_`：形状对象 → 只做路径解析（不创建实体）
- 否则：图片对象/瓦片 → 继续复用 `BasicEntityBuilder` 的构建逻辑

```cpp
// src/game/loader/entity_builder_mw.cpp（节选）
EntityBuilderMW* EntityBuilderMW::build() {
    if (object_json_ && !tile_info_) {
        buildPath();
    } else {
        BasicEntityBuilder::build();
    }
    return this;
}
```

### 3.2 buildPath：把 point 对象转成 WaypointNode

核心解析逻辑：

1) 只处理 `point == true` 的对象  
2) 取对象的 `id / x / y`  
3) 遍历 `properties[]`：  
   - `type == "object"` 且 `name` 以 `next` 开头 → 记入 `next_node_ids`  
   - `name == "start"` 且值为 `true` → 把 id 放进 `start_points_`  
4) 写入 `waypoint_nodes_[id] = WaypointNode{...}`  

```cpp
// src/game/loader/entity_builder_mw.cpp（节选）
if (object_json_->value("point", false) != true) return;
auto id = object_json_->value("id", 0);
auto position = glm::vec2(object_json_->value("x", 0.0f), object_json_->value("y", 0.0f));

for (auto& property : object_json_->at("properties")) {
    if (property.value("type", "") == "object" && property.value("name", "").starts_with("next")) {
        next_node_ids.push_back(property.value("value", 0));
    }
    if (property.value("name", "") == "start" && property.value("value", false) == true) {
        start_points_.push_back(id);
    }
}
waypoint_nodes_[id] = game::data::WaypointNode{id, std::move(position), std::move(next_node_ids)};
```

### 3.3 GameScene 接入：把自定义 Builder 注入 LevelLoader

`GameScene::loadLevel()` 中通过 `setEntityBuilder()` 把 Builder 换成游戏侧扩展版本：

```cpp
// src/game/scene/game_scene.cpp（节选）
engine::loader::LevelLoader level_loader;
level_loader.setEntityBuilder(std::make_unique<game::loader::EntityBuilderMW>(
    level_loader, context_, registry_, waypoint_nodes_, start_points_));
level_loader.loadLevel("assets/maps/level1.tmj", this);
```

这样关卡加载完毕时，我们就同时拥有：

- 地图/装饰等 ECS 实体（由引擎 Builder 负责）
- 路径节点表 `waypoint_nodes_` 与起点列表 `start_points_`（由游戏 Builder 负责）

## 🧟 4) 敌人组件：目标节点 id + 速度

敌人需要知道两件事：

1) 当前要走向哪个目标节点  
2) 以多快的速度移动  

于是引入 `EnemyComponent`：

```cpp
// src/game/component/enemy_component.h
struct EnemyComponent {
    int target_waypoint_id_;
    float speed_;
};
```

## 🧭 5) FollowPathSystem：把“路径节点”转换成 Velocity（让 MovementSystem 接管位移）

这一节的关键系统是 `FollowPathSystem`：它不直接改位置，而是计算“朝哪个方向走”，并更新 `VelocityComponent`。

筛选条件也很清晰：必须同时拥有

- `TransformComponent`（当前位置）
- `VelocityComponent`（我们要写的速度）
- `EnemyComponent`（目标节点与速度）

```cpp
// src/game/system/followpath_system.cpp（节选）
auto view = registry.view<engine::component::VelocityComponent,
                          engine::component::TransformComponent,
                          game::component::EnemyComponent>();
```

每个敌人的更新逻辑：

1) 根据 `enemy.target_waypoint_id_` 找到目标节点  
2) `direction = target.position - transform.position`  
3) 若距离小于阈值：到达节点  
   - 如果 `next_node_ids` 为空：到达终点 → 发送事件 + 标记死亡  
   - 否则：从 `next_node_ids` 随机选一个新目标  
4) 速度更新：`velocity = normalize(direction) * speed`  

```cpp
// src/game/system/followpath_system.cpp（节选）
if (glm::length(direction) < 5.0f) {
    if (target_node.next_node_ids_.empty()) {
        dispatcher.enqueue<game::defs::EnemyArriveHomeEvent>();
        registry.emplace<game::defs::DeadTag>(entity);
        continue;
    }
    auto target_index = engine::utils::randomInt(0, size - 1);
    enemy.target_waypoint_id_ = target_node.next_node_ids_[target_index];
}
velocity.velocity_ = glm::normalize(direction) * enemy.speed_;
```

注意两点：

- **阈值不要太小**：敌人速度快时可能“越过目标点来回抖动”
- 分叉路径的“随机选择”用到了新增的工具函数 `engine::utils::randomInt(min, max)`

## 🧹 6) 延迟删除：DeadTag + RemoveDeadSystem

当敌人到达终点时，我们做了两件事：

1) `dispatcher.enqueue<EnemyArriveHomeEvent>()`：发一个事件（后续加基地扣血/失败判定）  
2) `registry.emplace<DeadTag>(entity)`：先打上“死亡标签”，不要立刻 `destroy`  

`DeadTag` 是一个空结构体，用作 EnTT 的标签组件（零开销）：

```cpp
// src/game/defs/tags.h
struct DeadTag {};
```

然后专门用一个系统在“下一帧开头”清理：

```cpp
// src/game/system/remove_dead_system.cpp（节选）
auto view = registry.view<game::defs::DeadTag>();
for (auto entity : view) {
    registry.destroy(entity);
}
```

这样做的好处是：

- 避免在系统遍历 view 的过程中直接销毁实体导致迭代器问题
- 事件是 `enqueue` 的，会在 `dispatcher.update()` 时统一分发；“下一帧清理”更符合事件队列的时序

## 🎮 7) GameScene 串起来：加载路径 → 创建测试敌人 → 系统顺序

`GameScene::init()` 里新增了三件事：

- 初始化事件连接（监听 `EnemyArriveHomeEvent`）
- 创建测试敌人（每个起点来一个）
- 把新系统插入 update 链路

### 7.1 事件连接：敌人到家了先打日志

```cpp
// src/game/scene/game_scene.cpp（节选）
dispatcher.sink<game::defs::EnemyArriveHomeEvent>()
          .connect<&GameScene::onEnemyArriveHome>(this);
```

### 7.2 创建测试敌人：从 start_points_ 出发

```cpp
// src/game/scene/game_scene.cpp（节选）
for (auto start_index : start_points_) {
    auto position = waypoint_nodes_[start_index].position_;
    auto enemy = registry_.create();
    registry_.emplace<engine::component::TransformComponent>(enemy, position);
    registry_.emplace<engine::component::VelocityComponent>(enemy, glm::vec2(0, 0));
    registry_.emplace<game::component::EnemyComponent>(enemy, start_index, 100.0f);
}
```

并顺手给它补上渲染所需的组件（Sprite + RenderComponent）。其中 Sprite 的 `offset` 用来调整“锚点”，让敌人的逻辑位置更贴近脚底/中心（方便路径点对齐）：

```cpp
auto sprite = engine::component::Sprite("assets/textures/Enemy/wolf.png", engine::utils::Rect{0, 0, 192, 192});
registry_.emplace<engine::component::SpriteComponent>(enemy, std::move(sprite), glm::vec2(192, 192), glm::vec2(-96, -128));
registry_.emplace<engine::component::RenderComponent>(enemy, 10);
```

### 7.3 系统顺序：FollowPath → Movement → YSort

这一节我们把系统顺序明确下来（非常关键）：

1) **先清理死亡实体**（上一帧标记的 DeadTag）  
2) **FollowPathSystem** 更新速度（依赖当前位置）  
3) **MovementSystem** 更新位置（依赖速度）  
4) **YSortSystem** 更新深度（依赖新位置）  

```cpp
remove_dead_system_->update(registry_);
follow_path_system_->update(registry_, dispatcher, waypoint_nodes_);
movement_system_->update(registry_, delta_time);
animation_system_->update(registry_, delta_time);
ysort_system_->update(registry_);
```

## ✅ 本节小结

- 用 `WaypointNode + unordered_map` 表示分叉路径，并用 `start_points_` 支持多起点
- 在 Tiled 用 point 对象画节点，用 `next/next2...`（object 类型）表示连线，用 `start` 标记起点
- 通过自定义 Builder `EntityBuilderMW` 在关卡加载阶段解析路径点对象
- `FollowPathSystem` 计算方向并更新 `VelocityComponent`，让 MovementSystem 驱动位移
- 用 `DeadTag + RemoveDeadSystem` 实现延迟删除；终点触发 `EnemyArriveHomeEvent`

## 🔍 自检清单

- [ ] `waypoint_nodes_` 有数据（可在日志里观察 size）
- [ ] 起点列表 `start_points_` 不为空（会生成测试敌人）
- [ ] 敌人能沿路径移动，并在节点处平滑切换目标
- [ ] 分叉节点会随机选路（多次运行观察路线差异）
- [ ] 敌人到达终点会触发 “敌人到达基地” 日志，并在下一帧被清理

## ➡️ 下一节预告

下一节（第 15 节：实体工厂与蓝图）会把“创建敌人/玩家/建筑”的逻辑从 `GameScene` 中抽出来，做成数据驱动的 **EntityFactory + Blueprint**：用 JSON 定义单位属性与组件组合，让平衡和扩展都更轻松。
