# 12 关卡载入器

<div class="video-container">
  <div id="bilibili" class="video-content">
    <!-- B站嵌入：使用 https 明确协议（避免 file:// 或 http 导致被浏览器拦截） -->
    <iframe
      class="video-frame"
      src="https://player.bilibili.com/player.html?bvid=BV1762TBwEtR&page=1&autoplay=0&danmaku=0&high_quality=1"
      width="100%"
      height="480"
      scrolling="no"
      frameborder="0"
      allowfullscreen>
    </iframe>
  </div>
</div>

[在 Bilibili 上观看](https://www.bilibili.com/video/BV1762TBwEtR)

## 📖 概述

上一节我们把框架从 `GameObject + Component` 切换到了 ECS（`entt::registry` + 组件 + 系统），但此时的游戏世界还是“空”的：我们只能在 `GameScene` 里手动创建几个 entity 测试渲染与动画。

从这一节开始，我们进入塔防真正的“数据驱动”基础设施：**关卡载入器（LevelLoader）**。目标是：

- 用 **Tiled** 编辑地图
- 导出 `.tmj/.tsj`（JSON）
- 运行时解析 JSON，并把“地图/背景/装饰/对象”转换成 ECS 的实体与组件

![](../本期参考/PPT截图/怪物战争.055.png)

> PPT 第 55 页：第 12 节标题页

本节对应代码标签：`12-关卡载入器`（基线：`11-引入ECS框架`）。

## 🎯 学习目标

- 理解 Tiled 导出的 `.tmj/.tsj` 里最关键的字段：`tilesets / layers / (tile)gid`
- 实现 `LevelLoader::loadLevel()`：加载 tileset、分发三类图层解析
- 引入建造者模式：用 `BasicEntityBuilder` 把“解析”与“建实体”解耦
- 为瓦片层新增容器组件 `TileLayerComponent`，并支持瓦片动画的解析

## 🗺️ 关卡数据：Tiled 的 `.tmj/.tsj`（我们要读什么）

### 1) `.tmj`：地图文件（layers + tilesets）

Tiled 的 `.tmj` 本质是一个 JSON：里面有地图尺寸、瓦片尺寸、引用的 tileset，以及一个 `layers` 数组。

以 `assets/maps/level1.tmj` 为例，它开头就能看到第一个 `tilelayer` 的 `data`（一维数组，按行从左到右）：

```json
{ "height":19,
  "layers":[
    {
      "data":[0, 0, 0, 0, ...],
      "name":"ground1",
      "type":"tilelayer",
      "visible":true,
      "width":25
    }
  ]
}
```

而 `objectgroup`（对象层）会在 `objects` 里给出每个对象的 `x/y/width/height`，以及可选的 `gid`：

```json
{
  "name":"deco1",
  "type":"objectgroup",
  "objects":[
    {
      "gid":2147484301,
      "width":128,
      "height":128,
      "x":778.666666666667,
      "y":878.666666666667
    }
  ]
}
```

注意这里的 `gid`：它不仅仅是“图块 ID”，高位还可能编码了翻转/旋转等 flag（例如 `2147484301`）。**本节先不处理这些 flag**，下一节会专门完善。

### 2) `.tsj`：图块集文件（单图集 / 多图集）

tileset 有两类常见形态：

- **单图集**：整个 tileset 由一张大图（atlas）组成，tileset JSON 里有 `image/columns/tilewidth/tileheight`
- **多图集**：每个 tile 都是一张独立图片（或带裁剪信息），tileset JSON 里会有 `tiles[]`，每个 tile 里有 `image/imagewidth/imageheight/...`

本节的瓦片动画也来自 tileset（单图集情况下），例如 `assets/maps/tileset/Tilemap.tsj`：

```json
{
  "columns":20,
  "image":"..\/..\/textures\/Terrain\/Tilemap.png",
  "tileheight":64,
  "tiles":[
    {
      "id":81,
      "animation":[
        { "duration":100, "tileid":560 },
        { "duration":100, "tileid":561 }
      ]
    }
  ]
}
```

## 🧱 架构：LevelLoader + Builder（把“解析”与“建实体”拆开）

这节引入了一个很关键的结构设计：**建造者模式**。

![](../本期参考/PPT截图/怪物战争.056.png)

> PPT 第 56 页：LevelLoader 负责解析三类图层与 tileset，BasicEntityBuilder 负责配置并建造 ECS 实体（可替换/可继承扩展）

对应到代码：

- `src/engine/loader/level_loader.h/.cpp`：负责读取 `.tmj/.tsj`，并按图层类型组织解析流程
- `src/engine/loader/basic_entity_builder.h/.cpp`：把解析得到的 `TileInfo / object_json` 转成 entity + components

这样做的收益是：`LevelLoader` 可以保持“只管解析”，而游戏侧如果要“看到特定对象就加特定组件/生成特定实体”，只需要继承一个 Builder 并通过 `setEntityBuilder()` 替换即可。

## 🔧 LevelLoader：总流程（loadLevel）

`LevelLoader` 内部记录了 map 的关键信息（地图路径/尺寸/瓦片尺寸）、加载到的 tileset 数据，以及一个可替换的 `entity_builder_`：

```cpp
// src/engine/loader/level_loader.h（节选）
class LevelLoader final {
private:
    engine::scene::Scene* scene_;
    std::string map_path_;
    glm::ivec2 map_size_;
    glm::ivec2 tile_size_;
    std::map<int, nlohmann::json> tileset_data_; // firstgid -> tileset json
    std::unique_ptr<BasicEntityBuilder> entity_builder_;
    // ...
};
```

`loadLevel()` 的主体可以按 5 个步骤理解：

1) 校验 `scene`；若未指定 builder，则创建默认 `BasicEntityBuilder`  
2) 读取 `.tmj` 并解析 JSON  
3) 读取地图基本信息：`map_size_ / tile_size_ / map_path_`  
4) 遍历 `tilesets[]`，加载每个 `.tsj`，并按 `firstgid` 存入 `tileset_data_`  
5) 遍历 `layers[]`，按 `type` 分发到三种加载函数  

关键分发逻辑如下：

```cpp
// src/engine/loader/level_loader.cpp（节选）
for (const auto& layer_json : json_data["layers"]) {
    std::string layer_type = layer_json.value("type", "none");
    if (!layer_json.value("visible", true)) {
        spdlog::info("图层 '{}' 不可见，跳过加载。", layer_json.value("name", "Unnamed"));
        continue;
    }
    if (layer_type == "imagelayer") {
        loadImageLayer(layer_json);
    } else if (layer_type == "tilelayer") {
        loadTileLayer(layer_json);
    } else if (layer_type == "objectgroup") {
        loadObjectLayer(layer_json);
    } else {
        spdlog::warn("不支持的图层类型: {}", layer_type);
    }
}
```

## 🧩 三类图层如何转成 ECS

### 1) ImageLayer：背景图（含视差）

ImageLayer 的目标是创建一个带 `SpriteComponent` 的实体，并加上视差信息（`ParallaxComponent`）与偏移（`TransformComponent`）：

```cpp
// src/engine/loader/level_loader.cpp（节选）
registry.emplace<engine::component::NameComponent>(entity, name_id, layer_name);
registry.emplace<engine::component::TransformComponent>(entity, offset);
registry.emplace<engine::component::ParallaxComponent>(entity, scroll_factor, repeat);
registry.emplace<engine::component::SpriteComponent>(entity, sprite);
```

这里的 `sprite` 通过 `ResourceManager::getTextureSize(...)` 获取纹理大小后，使用“整张图片”作为源矩形：

```cpp
auto texture_size = resource_manager.getTextureSize(entt::hashed_string(texture_path.c_str()), texture_path);
auto sprite = engine::component::Sprite(texture_path, engine::utils::Rect{glm::vec2(0.0f), texture_size});
```

### 2) TileLayer：瓦片图层（每个瓦片都是一个 entity）

瓦片层是塔防地图的主体。本节的做法是：

- 先创建一个“图层实体”（只负责承载容器组件与名字）
- 再为每个非 0 的 `gid` 创建一个“瓦片实体”（有 `Sprite/Transform`，可选 `Animation`）
- 最后把所有瓦片实体装进图层实体的 `TileLayerComponent`

```cpp
// src/engine/loader/level_loader.cpp（节选）
auto layer_entity = registry.create();
registry.emplace<engine::component::NameComponent>(layer_entity, name_id, layer_name);

std::vector<entt::entity> tiles;
for (const int gid : data) {
    if (gid == 0) { index++; continue; }
    auto tile_info = getTileInfoByGid(gid);
    auto tile_entity = entity_builder_->configure(index, &tile_info.value())->build()->getEntityID();
    tiles.push_back(tile_entity);
    index++;
}
registry.emplace<engine::component::TileLayerComponent>(layer_entity, tile_size_, map_size_, tiles);
```

这里引入了一个“临时瓦片数据”结构 `TileInfo`（只是 LevelLoader 解析过程的中间产物，不会被长期保存）：

```cpp
// src/engine/component/tilelayer_component.h（节选）
struct TileInfo {
    engine::component::Sprite sprite_;
    engine::component::TileType type_;
    std::optional<engine::component::Animation> animation_;
    std::optional<nlohmann::json> properties_;
};
```

### 3) ObjectLayer：对象图层（形状对象 / 图片对象）

对象层的每个 object 都可能有 `gid`：

- `gid == 0`：代表“自己画的形状”（本节先按“只有 Transform/Name”处理）
- `gid != 0`：代表“图片对象”（本质上也是一个 tile 的引用，需要先用 `gid -> TileInfo`）

```cpp
// src/engine/loader/level_loader.cpp（节选）
auto gid = object.value("gid", 0);
if (gid == 0) {
    entity_builder_->configure(&object)->build();
} else {
    auto tile_info = getTileInfoByGid(gid);
    entity_builder_->configure(&object, &tile_info.value())->build();
}
```

## 🧠 gid → TileInfo：如何找到“它属于哪个 tileset”

### 1) 按 `firstgid` 存 tileset：loadTileset

地图文件只记录 tileset 的 `source` 与 `firstgid`，因此我们要：

- 读取 `.tsj`
- 把 `tileset_json` 存起来
- 额外注入一个 `file_path`（后续解析图片相对路径需要）

```cpp
// src/engine/loader/level_loader.cpp（节选）
ts_json["file_path"] = tileset_path;
tileset_data_[first_gid] = std::move(ts_json);
```

### 2) 用 upper_bound 找到最近的 firstgid

给定一个 `gid`，我们需要找到满足 `firstgid <= gid` 的那个 tileset；实现上用 `std::map` 的 `upper_bound`：

```cpp
// src/engine/loader/level_loader.cpp（节选）
auto tileset_it = tileset_data_.upper_bound(gid);
if (tileset_it == tileset_data_.begin()) return std::nullopt;
--tileset_it;

auto local_id = gid - tileset_it->first; // gid -> tileset local id
```

然后就进入“单图集 / 多图集”两种解析分支。

### 2.5) 流程图：gid → TileInfo → ECS 实体

用一张“从数据到实体”的流程图，把本节最关键的链路串起来：

```text
Tiled 导出数据（.tmj/.tsj）
        |
        |  layer.data[] / object.gid
        v
     gid（全局 ID）
        |
        |  (本节暂未处理翻转/旋转 flag；下一节补齐)
        v
tileset_data_（firstgid -> tileset json）
        |
        |  upper_bound(gid) 找到最近的 firstgid
        v
tileset_json + local_id = gid - firstgid
        |
        |  单图集: image/columns/tilewidth -> src_rect
        |  多图集: tiles[] 里按 id 找 image
        v
TileInfo{ Sprite, type, animation?, properties? }
        |
        |  BasicEntityBuilder.configure(...)
        v
buildBase -> buildSprite -> buildTransform -> buildAnimation
        |
        v
ECS entity + components（交给系统渲染/更新）
```

### 3) 单图集：计算源矩形 + 支持动画

单图集 tileset 有 `image` 字段，我们可以用 `columns/tilewidth/tileheight` 直接算源矩形：

```cpp
// src/engine/loader/level_loader.cpp（节选）
auto texture_rect = getTextureRect(tileset, local_id);
auto texture_path = resolvePath(tileset["image"].get<std::string>(), file_path);
tile_info.sprite_ = engine::component::Sprite(texture_path, texture_rect);
```

如果某个 tile 有 `animation[]`，本节会把它解析成 `AnimationFrame` 列表，并写入 `tile_info.animation_`：

```cpp
// src/engine/loader/level_loader.cpp（节选）
if (tile_json.contains("animation") && is_single_image && tile_json["animation"].is_array()) {
    std::vector<engine::component::AnimationFrame> animation_frames;
    for (auto& frame : tile_json["animation"]) {
        float duration_ms = frame.value("duration", 100.0f);
        int id = frame.value("tileid", 0);
        auto frame_rect = getTextureRect(tileset, id);
        animation_frames.push_back(engine::component::AnimationFrame(frame_rect, duration_ms));
    }
    tile_info.animation_ = engine::component::Animation(std::move(animation_frames));
}
```

### 4) 多图集：从 tiles[] 里按 id 找到 image

多图集 tileset 没有 `image`，而是在 `tiles[]` 里给每个 tile 配 `image`。本节的做法是遍历 `tiles[]` 查找 `id == local_id`，然后用 `image/imagewidth/imageheight` 创建精灵。

## 🧭 resolvePath：把 Tiled 的相对路径变成可用的文件路径

Tiled 导出的 JSON 里经常是相对路径（还带 `..`），因此 LevelLoader 统一用 `resolvePath()` 来拼路径并 canonical 化：

```cpp
// src/engine/loader/level_loader.cpp（节选）
auto map_dir = std::filesystem::path(file_path).parent_path();
auto final_path = std::filesystem::canonical(map_dir / relative_path);
return final_path.string();
```

## 🏗️ BasicEntityBuilder：配置 → 构建 → 返回（可扩展）

Builder 的接口非常直白：`configure(...) -> build() -> getEntityID()`，并且提供三个 `configure` 重载，分别对应：

- 对象层的形状对象：`configure(object_json)`
- 对象层的图片对象：`configure(object_json, tile_info)`
- 瓦片层的 tile：`configure(index, tile_info)`

构建时按顺序添加组件：

```cpp
// src/engine/loader/basic_entity_builder.cpp（节选）
buildBase();
buildSprite();
buildTransform();
buildAnimation();
buildAudio();
```

其中最“容易踩坑”的是 Transform：

- 对象层的 `x/y` 在 Tiled 里往往是“左下角”，而我们希望统一成“左上角”，所以要做 `y - height` 的修正
- 图片对象还要按目标 `width/height` 与源图大小计算缩放
- 瓦片层实体则通过 `index` 反推 `(col,row)` 再乘以 `tile_size`

```cpp
// src/engine/loader/basic_entity_builder.cpp（节选）
position_ = glm::vec2(object_json_->value("x", 0.0f), object_json_->value("y", 0.0f));
dst_size_ = glm::vec2(object_json_->value("width", 0.0f), object_json_->value("height", 0.0f));
position_ = glm::vec2(position_.x, position_.y - dst_size_.y);

if (index_ >= 0) {
    position_ = glm::vec2((index_ % map_size.x) * tile_size.x,
                          (index_ / map_size.x) * tile_size.y);
}
registry_.emplace<engine::component::TransformComponent>(entity_id_, position_, scale, rotation);
```

`buildAnimation()` 则把 `tile_info_` 里解析好的动画塞进 `AnimationComponent`（动画名默认用 `"tile"`）：

```cpp
// src/engine/loader/basic_entity_builder.cpp（节选）
auto animation_id = entt::hashed_string("tile");
animations.emplace(animation_id, std::move(tile_info_->animation_.value()));
registry_.emplace<engine::component::AnimationComponent>(entity_id_, std::move(animations), animation_id);
```

## 🎮 GameScene 接入：真正把地图“搬进”游戏

最后在 `GameScene::init()` 中调用 `loadLevel()`，把 `assets/maps/level1.tmj` 解析成 ECS 实体：

```cpp
// src/game/scene/game_scene.cpp（节选）
engine::loader::LevelLoader level_loader;
if (!level_loader.loadLevel("assets/maps/level1.tmj", this)) {
    spdlog::error("加载关卡失败");
    return false;
}
```

从这一刻起：地图瓦片/背景/对象不再需要硬编码创建，`RenderSystem` 会像渲染普通实体一样渲染它们（因为它们都有 `TransformComponent + SpriteComponent`，动画瓦片还有 `AnimationComponent`）。

## 🧩 顺手调整：窗口与逻辑分辨率的缩放（为了更好地看见地图）

为了让关卡在屏幕里更“像像素游戏”，配置文件新增了两个缩放参数：

- `window_scale`：实际窗口大小缩放
- `logical_scale`：SDL 逻辑分辨率缩放（影响像素密度与 letterbox）

对应字段在 `assets/config.json`、`src/engine/core/config.*` 与 `GameApp::initSDL()` 中被串了起来。

## ✅ 本节小结

- 引入 `LevelLoader`：读取 `.tmj/.tsj`，按 `imagelayer/tilelayer/objectgroup` 三类图层加载
- 引入 `BasicEntityBuilder`：把解析与“建实体”解耦，可替换/可继承扩展
- 为瓦片层新增 `TileLayerComponent`，每个瓦片都是独立 entity（可选动画）
- 实现 `gid -> tileset(firstgid) -> local_id -> TileInfo`，并支持单图集瓦片动画解析

## 🔍 自检清单

- [ ] 运行后日志出现 “`关卡加载完成: assets/maps/level1.tmj`”
- [ ] 背景图层能显示，并且 `parallaxx/parallaxy` 调整后有效果（有视差感）
- [ ] 瓦片层能正常铺出来（`gid == 0` 的格子不会创建实体）
- [ ] 对象层的图片对象显示位置大体正确（注意 y 方向已经做了修正）
- [ ] 存在动画的瓦片能动起来（例如 `Tilemap.tsj` 里带 `animation` 的 tile）

## ➡️ 下一节预告

本节我们已经能把关卡“载入并显示出来”，但还留了几个必补的坑：

- `gid` 的高位翻转/旋转 flag（例如 `2147484301`）目前没有处理
- 更精细的渲染顺序/背景色等（让画面更稳定、更可控）

这些会在下一节（第 13 节：完善关卡载入器）补齐。