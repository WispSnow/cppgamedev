...
- **向后衔接**：[事件系统](/courses/opengl-tiny-farm/parts/part-06)讲事件系统时，你会继续用它作为事件时序的验证工具。
...
> **工程约定**：调试 UI 与游戏输入共享同一事件源，但必须有明确的占用边界。这条规则在后续涉及 UI 框架（[UI 框架基础](/courses/opengl-tiny-farm/parts/part-17)）和输入系统（[输入系统](/courses/opengl-tiny-farm/parts/part-15)）的课程中会再次出现。
